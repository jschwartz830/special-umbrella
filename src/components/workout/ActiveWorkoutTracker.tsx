...
export function ActiveWorkoutTracker({
  ...
  const activeSetElapsed = activeSetTimer
    ? exercises[activeSetTimer.exIdx]?.sets[activeSetTimer.setIdx]?.setElapsedSeconds ?? null
    : null
      ...
      >
        <div className="grid grid-cols-3 divide-x divide-slate-700/60 py-2.5">
          ...
          <div className="flex flex-col items-center justify-center gap-1.5 px-2">
            <p className="text-[9px] text-slate-500 uppercase tracking-wider font-semibold">Set</p>
            <p className={`text-lg font-mono font-bold leading-none ${activeSetElapsed != null ? 'text-amber-300' : 'text-slate-600'}`}>
              {activeSetElapsed != null ? fmt(activeSetElapsed) : '—'}
            </p>
          </div>
        </div>
                ...
                return (
                  ...
                  >
                    ...
                    <input
                      ...
                      className="col-span-2 min-w-0 bg-slate-700 border border-slate-600 rounded px-1.5 py-1 text-xs text-slate-100 text-center"
                    />
                    <input
                      type="number"
                      min="0"
                    ...
                    <button
                      ...
                      className={`col-span-3 h-7 max-w-[84px] w-full min-w-0 justify-self-center flex items-center justify-center gap-1 rounded border font-mono text-[10px] transition-colors ${
                        timerRunning
                    ...
                    <button
                      ...
                      className={`col-span-1 h-7 w-7 justify-self-center flex items-center justify-center rounded border transition-colors ${
                        s.completed
