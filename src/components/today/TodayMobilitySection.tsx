...
export function TodayMobilitySection({
  ...
  const remainingCount = mobilityCompletion
    ? mobilityRoutine.filter(ex => !mobilityCompletion.completedExerciseIds.includes(ex.id)).length
    : 0
  return (
    <section>
      ...
      {mobilityRoutine.length === 0 ? (
        ...
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-teal-500/12 border border-teal-500/30">
          ...
          {remainingCount > 0 ? (
            ...
            >
              ...
              <p className="text-xs text-teal-300/60 mt-0.5">
                ...
                tap to finish {remainingCount} more
              </p>
            </button>
          ) : (
          ...
          {remainingCount > 0 && (
            <button
              onClick={onResumeCompletion}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-teal-500 hover:bg-teal-600 text-white font-semibold text-xs transition-colors active:scale-[0.98] flex-shrink-0"
            >
          ...
          <button
            ...
            className="text-teal-400/50 hover:text-teal-200 transition-colors flex-shrink-0"
